    ...
    <section class="vbox">
        <section class="scrollable padder">
            <ul class="breadcrumb no-border no-radius b-b b-light pull-in">
                ...
                <li><a href="<%=request.getContextPath()%>/category/manual/list.html">Manual</a></li> <li><a href="javascript:void(0)">Cập nhập tài liệu</a></li>
            </ul>
            <div class="m-b-md">
                <h3 class="m-b-none">Cập nhập tài liệu </h3>
            </div>

            <section class="panel panel-default">
                <header class="panel-heading"><i class="fa fa-caret-square-o-right"></i>Cập nhập tài liệu "<s:property value="item.title" />"</header>
                <div class="panel-body" style="overflow: scroll;min-height: 500px;">
                    <s:form method="post" id="form-submit" action="edit-save" theme="simple"  enctype="multipart/form-data" cssStyle="">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <div class="input-group m-b" style="width: 100%">
                                    ...
                                    <s:textfield name="item.title" maxlength="200" cssStyle="width:100%;"
                        ...
                        <div class="form-group">
                            <label class="col-sm-2 control-label label-bam-trai">File đính kèm</label>
                            <div class="col-sm-5 themfile" style="padding-top:7px" id="removeFile">
                                <s:if test="%{item.fileName.equals('') == false && item.fileName != null}">

                                    <s:iterator var="fileName" value="listNameFile" status="stat">
                                        <div class="col-sm-12">
                                            File <s:property value="#stat.count"></s:property> : <a href="<%=request.getContextPath()%>/category/manual/download-file.html?id=<s:property value="item.id"/>&count=<s:property value="#stat.count"/> " class="check-number-file">
                                            <s:property value="#fileName"/> </a><a onclick="deleteFile(<s:property value="#stat.count" />)" style="margin-left: 20px">Gỡ bỏ</a>
                                        </div>
                                    </s:iterator>
                                </s:if>
...
<script>
    ...
    function deleteFile(index) {
        var deleteFile = <s:property value="item.id" /> ;

        $.ajax({
            type: "POST",

            url: '<%=request.getContextPath()%>/json/manual/remove-file.html?id=<s:property value="item.id"/>&count='+index,
            dataType: 'json',
...
<script>
    ...
    function themFile() {
        var count_input = $('input[name="fileUpload"]').length + $('.check-number-file').length ;
        if(count_input  < 3) {
            $(".themfile").append("<span><input style='float: left ;width: 350px' class='prevent-file_upload myFile' type='file' name='fileUpload'><a style='float: right' class='huy' >Hủy</a></span>");
